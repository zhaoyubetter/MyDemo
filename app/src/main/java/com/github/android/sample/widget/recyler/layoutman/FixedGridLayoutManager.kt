...
class FixedGridLayoutManager : RecyclerView.LayoutManager() {
    ...
    var mFirstChangedPosition = 0
    var mChangedPositionCount = 0
    ...
    private var mTotalColumnCount = 1
        ...
        if (dy > 0) { // Contents are scrolling up
            ...
            if (bottomBoundReached) {
                ...
                if (rowOfIndex(childCount - 1) >= maxRowCount - 1) {
                    ...
                    bottomOffset = getVerticalSpace() - getDecoratedBottom(bottomView) + paddingBottom
                } else {
                    /*
                     ...
                     */
                    bottomOffset = getVerticalSpace() - (getDecoratedBottom(bottomView) + mDecoratedChildHeight) + paddingBottom
                }
    ...
    override fun onLayoutChildren(recycler: RecyclerView.Recycler, state: RecyclerView.State?) {
        ...
        if (state?.isPreLayout != true) {        // false
        ...
        var removedCache: SparseIntArray = SparseIntArray()
        /*
        ...
        if (childCount == 0) {   // First or empty layout
            ...
            childTop = paddingTop
        } else if (state?.isPreLayout == false && getVisibleChildCount() >= state.itemCount) {
            ...
            * 未填充满
            */
            if (state?.isPreLayout == false &&
                    getVerticalSpace() > (getTotalRowCount() * mDecoratedChildHeight)) {
        ...
        if (state?.isPreLayout == false && !recycler.scrapList.isEmpty()) {
        ...
        for (i in 0 until getVisibleChildCount()) {
            ...
            if (state?.isPreLayout == true) {
                ...
                for (offset in 0 until (removedPositions?.size() ?: 0)) {
                    ...
                    if (removedPositions?.valueAt(offset) == REMOVE_INVISIBLE && removedPositions?.keyAt(offset) ?: 0 < nextPosition) {
            ...
            if (nextPosition < 0 || nextPosition >= state?.itemCount ?: 0) {
    ...
    private fun getLastVisibleColumn(): Int {
    ...
    override fun checkLayoutParams(lp: RecyclerView.LayoutParams?) = lp is LayoutParams
